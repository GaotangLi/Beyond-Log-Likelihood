...
experiment_name=numina-cot-qwen-2.5-72b-lr-$lr-bz-$bz-max_length-$max_length-nproc_per_node-$nproc_per_node-micro_batch_size-$micro_batch_size-1113-p
save_path=./checkpoints/ablation/model_scale/$experiment_name
...
CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES torchrun --standalone --nnodes=1 --nproc_per_node=$nproc_per_node \
    ...
    model.partial_pretrain=Qwen/Qwen2.5-72B \
...
MODEL_SAVE_NAME="qwen-2.5-72b"
MODEL_NAME_OR_PATH="$save_path/global_step_264"
OUTPUT_DIR="./results/ablation/model_scale/$MODEL_SAVE_NAME/$experiment_name"
n_sampling=16
temperature=1
