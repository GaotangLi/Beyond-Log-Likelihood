            ...
            if not use_sp:
                ...
                if self.config.trainer.objective_trans == "original":
                    ...
                    if self.config.trainer.objective_trans == "p": 
                        ...
                        loss = loss * loss_mask.to(loss.device)
                    elif self.config.trainer.objective_trans.startswith("convexity-"):
                        q = float(self.config.trainer.objective_trans.split("-")[1])
                        loss = - torch.exp(q * log_p_true.float()) / q
                        loss = loss * loss_mask.to(loss.device)
                        ...
                        loss = loss_fct(shift_logits, shift_labels) # Original implementation 
                        loss = loss * loss_mask.to(loss.device)
                    elif self.config.trainer.objective_trans.startswith("UnlikehoodTop-"):
                        top_th = float(self.config.trainer.objective_trans.split("-")[1])
                        assert top_th >= 0 and top_th < 1, "top_th must be between 0 and 1"
                        loss_mask = loss_mask * (prob_coefficients >= top_th)
                        loss = torch.log(1.0 - prob_coefficients + 1e-3)
                        loss = loss * loss_mask.to(loss.device)
                    elif self.config.trainer.objective_trans.startswith("UnlikehoodBottom-"):
                        bottom_th = float(self.config.trainer.objective_trans.split("-")[1])
                        assert bottom_th > 0 and bottom_th <= 1, "bottom_th must be between 0 and 1"
                        loss_mask = loss_mask * (prob_coefficients <= bottom_th)
                        loss = torch.log(1.0 - prob_coefficients + 1e-3)
